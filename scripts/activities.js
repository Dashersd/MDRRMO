...
(function() {
  ...
  function createActivityCard(activity) {
    ...
    card.style.cursor = 'pointer'; // Show that the entire card is clickable
    ...
    if (activity.images && activity.images.length > 0) {
      ...
      activity.images.forEach((imageUrl, index) => {
        ...
        thumb.addEventListener('click', function(e) {
          ...
          openImageGallery(activity, index);
        });
    ...
    card.addEventListener('click', function(e) {
      // If user clicked inside a thumbnail, let the thumbnail click listener handle it
      if (e.target.closest('.activity-image-thumb')) {
        return;
      }
